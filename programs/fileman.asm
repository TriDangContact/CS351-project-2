...
.undelete_file:
	call .draw_background
	mov bx, 1				; set undelete flag
	call os_file_selector			; Get filename from user
	jc .no_undelete_file_selected		; If Esc pressed, quit out
	
	mov si, ax
	mov di, .undeletename_tmp1
	call os_string_copy
...
.retry_undelete:
	...
	mov di, .undeletename_tmp2
	call os_string_copy

	mov ax, di				; Does the second filename already exist?
	call os_file_exists
	jnc .undelete_fail			; Quit out if so

	mov ax, .undeletename_tmp1
	mov bx, .undeletename_tmp2
...
.draw_background:
	...
	.command_list		db 'Delete a file,Rename a file,Copy a file,Show file size,Show disk info,Undelete a file', 0
	...
	.footer_msg		db 'Copy, rename, delete, and undelete files', 0
	...
	.undeletename_tmp1	times 15 db 0		;tri modify
	.undeletename_tmp2	times 15 db 0		;tri modify
