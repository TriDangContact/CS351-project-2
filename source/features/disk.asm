...
; IN/OUT: AX = location to store zero-terminated filename string, IN: BX = undelete flag (1 = set)

os_get_file_list:
	pusha
	mov word [.undelete_flag], bx	;tri modify, get undelete flag
...
;tri modify----------------------------------------------------------
;determines if we need to show deleted files or not based on flag that was passed in
	mov bx, [.undelete_flag]
	cmp bx, 1
	jne .no_show_deleted		;if undelete flag not set, don't show deleted files
.show_deleted:
	cmp al, 41h			;if filename doesn't contain letter (deleted), then continue
	jb .continue			;otherwise skip
	cmp al, 5Ah
	ja .continue			
		
	jmp .skip
.no_show_deleted:
	cmp al, 2Ah			; If we read asterisk = deleted filename
	je .skip	
...
os_remove_file:
	...
	mov byte [di], 2Ah		; tri modify, Mark directory entry (first byte of filename) as asterisk (original = 0E5h)
